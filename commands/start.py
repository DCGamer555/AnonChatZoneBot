...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all([init.user_details[user_id].get("gender"), init.user_details[user_id].get("age"), init.user_details[user_id].get("country")]):
        return
    await safe_tele_func_call(update.message.reply_text, text="👋 Welcome back to *Chat Zone - Anonymous Chat Bot!* Use /find to look for a partner.", parse_mode="Markdown")
